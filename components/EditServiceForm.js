...
import { Edit, Globe, Type, Build, Server } from 'lucide-react';
import { updateService } from '../lib/springBootApi';

const EditServiceForm = ({ site, onServiceUpdated }) => {
    ...
    const [formData, setFormData] = useState({
        ...
        owner: site.description?.split(' (')[0] || '',
        criticality: site.description?.includes('High') ? 'HIGH' : site.description?.includes('Critical') ? 'CRITICAL' : 'NORMAL'
    });
    ...
    return (
        <>
            <button
                ...
                className="inline-flex items-center px-3 py-1.5 border border-indigo-600 text-xs font-medium rounded-lg text-indigo-600 bg-white hover:bg-indigo-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors"
                aria-label="Edit service node"
            >
                <Edit className="-ml-1 mr-1.5 h-3.5 w-3.5" />
                Edit
            </button>
                            ...
                            <div className="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4 border-b border-gray-200 dark:border-gray-700">
                                <div className="sm:flex sm:items-start">
                                    <div className="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-indigo-100 dark:bg-indigo-900/50 sm:mx-0 sm:h-10 sm:w-10">
                                        <Edit className="h-6 w-6 sm:h-5 sm:w-5 text-indigo-600 dark:text-indigo-400" />
                                    </div>
                                ...
                                <div className="px-4 py-5 sm:p-6 space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="relative rounded-md shadow-sm">
                                                ...
                                                <input
                                                    required
                                                    type="text"
                                                    className="focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-10 sm:text-sm border-gray-300 dark:border-gray-600 rounded-md dark:bg-gray-700 dark:text-white bg-white border py-2"
                                                    value={formData.name}
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <select
                                                className="focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 dark:border-gray-600 rounded-md dark:bg-gray-700 dark:text-white bg-white border py-2 px-3"
